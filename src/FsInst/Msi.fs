...
module Msi =
    ...
    let private addBasicSequences (database:Database) =
    ...
    let private writeSummaryInfo installationPackage (database:Database) =
        ...
        if not (List.isEmpty installationPackage.Folders) then
            ...
            for folder in installationPackage.Folders do
                ...
                for c in folder.Components do
                    ...
                    c.Files
                    |> List.iteri (fun i f ->
                        cabinet.PackFiles(".", [| f.FileName |], [| f.Id |], CompressionLevel.Max, null)
